...
COLOR_ERROR = 0x2980b9
COLOR_INFO = 0x85c1e9
COLOR_WARNING = 0xaed6f1
COLOR_DEPLOY = 0xe67e22
...
TIC_TAC_TOE_REWARD = 50
ROCK_PAPER_SCISSORS_REWARD = 30
NUMBER_GUESS_REWARD = 40
TRIVIA_REWARD = 35

TRIAL_VPS_DURATION_DAYS = 3
...
OS_OPTIONS = {
    ...
    "fedora41": {
        ...
        "emoji": "🔵",
        "description": "Cutting-edge Fedora with latest GNOME & libs",
        "arch": "x86_64"
    },
    "fedora40": {
        "label": "Fedora 40",
        ...
        "emoji": "🔵",
        "description": "Stable Fedora with modern RPM packages",
        "arch": "x86_64"
    },
    "fedora39": {
        "label": "Fedora 39",
        ...
        "emoji": "🔵",
        "description": "Previous Fedora release, well-tested",
        "arch": "x86_64"
    },
    "alpine320": {
        ...
        "emoji": "🏔️",
        "description": "Ultra-lightweight, ideal for containers & Docker",
        "arch": "x86_64"
    },
    "rocky9": {
        "label": "Rocky Linux 9",
        ...
        "emoji": "🪨",
        "description": "RHEL-compatible, enterprise-grade stability",
        "arch": "x86_64"
    },
    "almalinux9": {
        "label": "AlmaLinux 9",
        ...
        "emoji": "⭐",
        "description": "Community RHEL alternative, production ready",
        "arch": "x86_64"
    },
    "arch": {
        "label": "Arch Linux (Latest)",
    ...
    "centos9": {
        ...
        "emoji": "🎯",
        "description": "Upstream RHEL, continuous delivery model",
        "arch": "x86_64"
    },
    "opensuse": {
        "label": "openSUSE Leap 15.6",
        "image": "images:opensuse/15.6",
        "emoji": "🦎",
        "description": "Enterprise Linux based on SUSE, highly stable",
        "arch": "x86_64"
    }
}
...
async def core_create_container(container_name: str, ram_gb: int, cpu: int, storage_gb: int, storage_pool: str = None, os_image: str = None):
...
@is_admin()
async def deploy_custom_vps(ctx, member: discord.Member = None, ram: int = None, cpu: int = None, storage: int = None):
    if not member or ram is None or cpu is None or storage is None:
        await ctx.send(embed=create_error_embed(
            "Invalid Usage",
            "Usage: `.deploy @user <ram> <cpu> <storage>`\nExample: `.deploy @user 8 2 50`"
        ))
        return

    os_select_embed = discord.Embed(
        title="🖥️ Select Operating System",
        description=(
            f"**Deploying VPS for:** {member.mention}\n\n"
            f"**📋 Deployment Specs:**\n"
            f"> 🧠 **RAM:** {ram}GB\n"
            f"> ⚙️ **CPU:** {cpu} core{'s' if cpu > 1 else ''}\n"
            f"> 💾 **Storage:** {storage}GB\n\n"
            f"Please select the operating system you'd like to deploy from the dropdown below.\n"
            f"**⏳ This selection will expire in 60 seconds.**"
        ),
        color=COLOR_DEPLOY
    )
    os_select_embed.set_thumbnail(url=THUMBNAIL_IMAGE_URL)
    os_select_embed.add_field(
        name="🐧 Available Distributions",
        value=(
            "🟠 **Ubuntu** — 20.04, 22.04, 24.04 LTS\n"
            "🔴 **Debian** — 11, 12, 13\n"
            "🔵 **Fedora** — 39, 40, 41\n"
            "🏔️ **Alpine** — Lightweight container OS\n"
            "🪨 **Rocky Linux** — RHEL-compatible enterprise\n"
            "⭐ **AlmaLinux** — Community RHEL alternative\n"
            "🏹 **Arch Linux** — Rolling release\n"
            "🎯 **CentOS Stream** — Upstream RHEL\n"
            "🦎 **openSUSE** — Enterprise SUSE-based"
        ),
        inline=False
    )
    os_select_embed.add_field(
        name="ℹ️ Info",
        value="All images are official LXC images. Deployment will begin immediately after OS selection.",
        inline=False
    )
    os_select_embed.set_footer(
        text=f"{BOT_NAME} {BOT_VERSION} • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        icon_url=FOOTER_ICON_URL
    )
    ...
    class OSSelect(discord.ui.Select):
        def __init__(self):
            options = [
                discord.SelectOption(label="Ubuntu 20.04 LTS (Focal Fossa)", value="ubuntu2004", emoji="🟠", description="Long-term support, stable & widely supported"),
                discord.SelectOption(label="Ubuntu 22.04 LTS (Jammy Jellyfish)", value="ubuntu2204", emoji="🟠", description="Latest LTS, modern packages & security"),
                discord.SelectOption(label="Ubuntu 24.04 LTS (Noble Numbat)", value="ubuntu2404", emoji="🟠", description="Newest LTS release with cutting-edge features"),
                discord.SelectOption(label="Debian 13 (Trixie)", value="debian13", emoji="🔴", description="Testing branch, latest Debian packages"),
                discord.SelectOption(label="Debian 12 (Bookworm)", value="debian12", emoji="🔴", description="Current stable Debian, highly reliable"),
                discord.SelectOption(label="Debian 11 (Bullseye)", value="debian11", emoji="🔴", description="Older stable Debian, max compatibility"),
                discord.SelectOption(label="Fedora 41", value="fedora41", emoji="🔵", description="Cutting-edge Fedora with latest GNOME & libs"),
                discord.SelectOption(label="Fedora 40", value="fedora40", emoji="🔵", description="Stable Fedora with modern RPM packages"),
                discord.SelectOption(label="Fedora 39", value="fedora39", emoji="🔵", description="Previous Fedora release, well-tested"),
                discord.SelectOption(label="Alpine Linux 3.20", value="alpine320", emoji="🏔️", description="Ultra-lightweight, ideal for containers"),
                discord.SelectOption(label="Rocky Linux 9", value="rocky9", emoji="🪨", description="RHEL-compatible, enterprise-grade stability"),
                discord.SelectOption(label="AlmaLinux 9", value="almalinux9", emoji="⭐", description="Community RHEL alternative, production ready"),
                discord.SelectOption(label="Arch Linux (Latest)", value="arch", emoji="🏹", description="Rolling release, always up-to-date packages"),
                discord.SelectOption(label="CentOS Stream 9", value="centos9", emoji="🎯", description="Upstream RHEL, continuous delivery model"),
                discord.SelectOption(label="openSUSE Leap 15.6", value="opensuse", emoji="🦎", description="Enterprise Linux based on SUSE, highly stable"),
            ]
            super().__init__(
        ...
        async def callback(self, interaction: discord.Interaction):
            if interaction.user.id != ctx.author.id:
                await interaction.response.send_message("Only the admin who ran this command can select an OS!", ephemeral=True)
                return

            selected_key = self.values[0]
            os_info = OS_OPTIONS[selected_key]
            self.view.selected_os = selected_key
            self.view.stop()
            ...
            confirmed_embed = discord.Embed(
                title="🚀 Deploying VPS...",
                description=(
                    f"**OS Selected:** {os_info['emoji']} {os_info['label']}\n"
                    f"**For:** {member.mention}\n\n"
                    f"**📋 Specs:**\n"
                    f"> 🧠 **RAM:** {ram}GB\n"
                    f"> ⚙️ **CPU:** {cpu} core{'s' if cpu > 1 else ''}\n"
                    f"> 💾 **Storage:** {storage}GB\n\n"
                    f"⏳ Please wait while the container is being created..."
                ),
                color=COLOR_DEPLOY
            )
            confirmed_embed.set_thumbnail(url=THUMBNAIL_IMAGE_URL)
            confirmed_embed.set_footer(
                text=f"{BOT_NAME} {BOT_VERSION} • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                icon_url=FOOTER_ICON_URL
            )

            await interaction.response.edit_message(embed=confirmed_embed, view=self.view)
    ...
    view = OSSelectView()
    msg = await ctx.send(embed=os_select_embed, view=view)
    await view.wait()

    if view.selected_os is None:
        timeout_embed = discord.Embed(
            title="⏰ Selection Timed Out",
            description="OS selection expired. Please run `.deploy` again.",
            color=COLOR_ERROR
        )
        timeout_embed.set_footer(text=f"{BOT_NAME} {BOT_VERSION}", icon_url=FOOTER_ICON_URL)
        await msg.edit(embed=timeout_embed, view=view)
        return

    os_info = OS_OPTIONS[view.selected_os]
    ...
    try:
        ...
        success_embed = discord.Embed(
            ...
            description=f"The VPS has been deployed for {member.mention}.",
            color=COLOR_SUCCESS
        )
        success_embed.set_thumbnail(url=THUMBNAIL_IMAGE_URL)
        success_embed.add_field(name="🖥️ Operating System", value=f"{os_info['emoji']} {os_info['label']}", inline=False)
        success_embed.add_field(name="📦 Container", value=f"`{container_name}`", inline=True)
        success_embed.add_field(name="👤 Owner", value=member.mention, inline=True)
        success_embed.add_field(
            name="📋 Specifications",
            value=f"🧠 **RAM:** {ram}GB\n⚙️ **CPU:** {cpu} core{'s' if cpu > 1 else ''}\n💾 **Storage:** {storage}GB",
            inline=False
        )
        success_embed.add_field(name="🟢 Status", value="Running", inline=True)
        success_embed.add_field(name="📅 Created", value=datetime.now().strftime('%Y-%m-%d %H:%M'), inline=True)
        success_embed.set_footer(
            text=f"{BOT_NAME} {BOT_VERSION} • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            icon_url=FOOTER_ICON_URL
        )

        await msg.edit(embed=success_embed, view=view)

        try:
            user_embed = discord.Embed(
                title="✅ Your VPS is Ready!",
                description=f"An admin has deployed a VPS for you!",
                color=COLOR_SUCCESS
            )
            user_embed.set_thumbnail(url=THUMBNAIL_IMAGE_URL)
            user_embed.add_field(name="🖥️ Operating System", value=f"{os_info['emoji']} {os_info['label']}", inline=False)
            ...
            user_embed.add_field(name="🔧 Management", value="Use `.manage` to control your VPS", inline=False)
        ...
        error_embed = discord.Embed(
            ...
            description=f"Failed to deploy VPS for {member.mention}.\n\n**Error:** {str(e)[:200]}",
            color=COLOR_ERROR
        )
        error_embed.set_footer(text=f"{BOT_NAME} {BOT_VERSION}", icon_url=FOOTER_ICON_URL)
        await msg.edit(embed=error_embed, view=view)
...
async def list_all_vps(ctx, page: int = 1):
    ...
    for user_id, vps in current_page_vps:
        ...
        os_label = vps.get('os', 'Debian 12 (Default)')
    ...
    try:
        ...
        if action == 'start':
            ...
            try:
                ...
                embed.add_field(name="OS", value=vps.get('os', 'Debian 12 (Default)'), inline=True)
...
async def game_settings_cmd(ctx, setting: str = None, value: int = None):
    if not setting:
        ...
        embed.add_field(name="Tic-Tac-Toe Reward", value=f"{game_settings.get('tic_tac_toe_reward', TIC_TAC_TOE_REWARD)} credits", inline=True)
        embed.add_field(name="Rock Paper Scissors Reward", value=f"{game_settings.get('rock_paper_scissors_reward', ROCK_PAPER_SCISSORS_REWARD)} credits", inline=True)
        embed.add_field(name="Number Guess Reward", value=f"{game_settings.get('number_guess_reward', NUMBER_GUESS_REWARD)} credits", inline=True)
        embed.add_field(name="Trivia Reward", value=f"{game_settings.get('trivia_reward', TRIVIA_REWARD)} credits", inline=True)
        embed.add_field(
            name="How to Update",
...
async def show_help(ctx):
    ...
    if is_user_admin:
        admin_cmds = (
            ...
            "**.deploy @user ram cpu storage** - Deploy custom VPS with OS picker\n"
            "**.create @user [plan]** - Create VPS from plan\n"
